...
var supportedCommands = []BotCommand{
	NewBotCommand("launch <image_or_version_or_prs> <options>", &CommandDefinition{
		Description: fmt.Sprintf("Launch an OpenShift cluster using a known image, version, or PR(s). You may omit both arguments. Arguments can be specified as any number of comma-delimited values. Use `nightly` for the latest OCP build, `ci` for the the latest CI build, provide a version directly from any listed on https://amd64.ocp.releases.ci.openshift.org, a stream name (4.17.0-0.ci, 4.17.0-0.nightly, etc), a major/minor `X.Y` to load the \"next stable\" version, from nightly, for that version (`4.17`), `<org>/<repo>#<pr>` to launch from any combination of PRs, or an image for the first argument. Options is a comma-delimited list of variations including platform (%s), architecture (%s), and variant (%s).",
...
func TestMatch(t *testing.T) {
	...
	testCases := []struct {
		...
		command: "launch 4.17,openshift/installer#7160,openshift/machine-config-operator#3688 gcp,techpreview",
		match:   0,
		properties: &Properties{
			PropertyMap: map[string]string{
				"image_or_version_or_prs": "4.17,openshift/installer#7160,openshift/machine-config-operator#3688",
		...
		command:       "test upgrade 4.16 4.17 aws",
		match:         4,
		allowMultiple: true,
		properties: &Properties{
			PropertyMap: map[string]string{
		...
		command: "test e2e 4.17 vsphere",
		match:   5,
		properties: &Properties{
			PropertyMap: map[string]string{
				"name":                    "e2e",
				"image_or_version_or_prs": "4.17",
				"options":                 "vsphere",
			},
		},
	}, {
		command: "test e2e 4.17 alibaba",
		match:   5,
		properties: &Properties{
			PropertyMap: map[string]string{
				"name":                    "e2e",
		...
		command: "test e2e 4.17 azure-stackhub",
		match:   5,
		properties: &Properties{
			PropertyMap: map[string]string{
				"name":                    "e2e",
