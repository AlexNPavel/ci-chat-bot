...
This repo requires the `-tags gcs` build flag. The vendored `cyborg-data` package gates GCS client code behind this tag. Both the Makefile and `.Codex/settings.json` (via `GOFLAGS`) handle this automatically, so `make` commands and direct `go` commands within Codex work without any extra flags.

### Testing and Verification Guidelines for Codex
